...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-5d2f93cb";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-5d2f93cb">'+'<span class="tile-face-5d2f93cb tile-front-5d2f93cb"></span>'+'<span class="tile-face-5d2f93cb tile-back-5d2f93cb"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-5d2f93cb")){return;}
var back=tile.querySelector(".tile-back-5d2f93cb");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-5d2f93cb");if(isMine){tile.classList.add("boom-5d2f93cb");}
if(dim){tile.classList.add("dimmed-5d2f93cb");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-5d2f93cb")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-5d2f93cb");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-5d2f93cb")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-5d2f93cb");void panelEl.offsetWidth;panelEl.classList.add("jolt-5d2f93cb");}
endRound();}
